...
def filter_dataset(pd_data):
    ...
    tokenizer = CLIPTokenizer.from_pretrained("/home/majc/stable-diffusion-v1-4", subfolder="tokenizer", cache_dir=".cache")
    ignore = []
...
class PNGImageDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        if 'evaluation_guidance' not in prompts_df.columns:
            prompts_df['evaluation_guidance'] = 7.5
    ...
    def __getitem__(self, idx):
        ...
        seed = self.data.iloc[idx].evaluation_seed if 'evaluation_seed' in self.data.columns else -1
        ...
        return image,img_path,prompt, seed, guidance_scale
