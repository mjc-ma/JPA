...
class CustomTextEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        if self.eos_token_id == 2:
            ...
            a = torch.arange(last_hidden_state.shape[0], device=last_hidden_state.device)
            # print(torch.arange(last_hidden_state.shape[0], device=last_hidden_state.device))
            # print((input_ids.to(dtype=torch.int, device=last_hidden_state.device) == self.eos_token_id).int())
            # print((input_ids.to(dtype=torch.int, device=last_hidden_state.device) == self.eos_token_id)
            #     .int().argmax(dim=-1))
        else:
...
if __name__ == "__main__":
    dir_ = "/home/majc/stable-diffusion-v1-4"
